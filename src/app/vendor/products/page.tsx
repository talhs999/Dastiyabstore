...
export default function VendorProductsPage() {
  ...
  const fetchData = async () => {
    ...
    const [catRes, prodRes] = await Promise.all([
      ...
      fetch(`/api/vendor/products?store_id=${storeId}`)
    ]);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "var(--gray-900)", marginBottom: 4 }}>My Products</h1>
          <p style={{ fontSize: 14, color: "var(--gray-500)" }}>{products.length} products in your store</p>
        </div>
        <Link href="/vendor/products/new" className="btn-gray" style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--gray-900)", color: "white", padding: "10px 16px", border: "none", borderRadius: "var(--radius)", cursor: "pointer", fontWeight: 600, textDecoration: "none" }}>
      ...
      <div style={{ background: "white", borderRadius: "var(--radius-lg)", boxShadow: "var(--shadow-sm)", border: "1px solid var(--gray-200)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} style={{ padding: 40, textAlign: "center", color: "var(--gray-500)" }}>Loading products...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={5} style={{ padding: 40, textAlign: "center", color: "var(--gray-500)" }}>No products found.</td></tr>
            ) : filtered.map((p) => (
              ...
              >
                ...
                <td style={{ padding: "16px 24px" }}>
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <Link href={`/vendor/products/${p.id}`} style={{ width: 32, height: 32, borderRadius: 8, background: "white", border: "1px solid var(--gray-200)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--gray-900)", transition: "all 0.2s" }} title="Edit">
                      <Edit size={15} />
                    </Link>
