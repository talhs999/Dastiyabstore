...
export default function EditBundlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (prodRes.ok) {
        ...
        setAllProducts(allProds.filter((p: any) => !p.is_bundle));
      }

      // Fetch the bundle
      const bundleRes = await fetch(`/api/products?ids=${id}`);
      if (bundleRes.ok) {
        ...
        if (bundles.length > 0) {
          ...
          setFormData({
            ...
            bundle_items: b.bundle_items || [],
            bundle_discount_pc: b.bundle_discount_pc?.toString() || "0"
          });
        }
      }
  ...
  if (pageLoading) return <div style={{ padding: 40, textAlign: "center" }}>Loading bundle data...</div>;
  ...
  return (
    <div style={{ padding: "32px 40px", maxWidth: 900 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 32 }}>
        ...
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "var(--gray-900)", marginBottom: 4 }}>Edit Bundle</h1>
          <p style={{ fontSize: 14, color: "var(--gray-500)" }}>Modify your existing promotional bundle</p>
        </div>
      </div>
