...
export default function AdminBundlesPage() {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    const res = await fetch("/api/admin/products?is_bundle=true");
  ...
  return (
    <div style={{ padding: "32px 40px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 900, color: "var(--gray-900)", marginBottom: 4 }}>Bundles</h1>
          <p style={{ fontSize: 14, color: "var(--gray-500)" }}>{products.length} bundles in your store</p>
        </div>
        <Link href="/admin/bundles/new" className="btn-gray" style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--gray-900)", color: "white", padding: "10px 16px", border: "none", borderRadius: "var(--radius)", cursor: "pointer", fontWeight: 600, textDecoration: "none" }}>
              ...
              >
                <td style={{ padding: "16px 24px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    <img src={p.image || "https://placehold.co/100x100?text=No+Image"} alt={p.name} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover", border: "1px solid var(--gray-200)" }} />
                    <div>
                ...
                <td style={{ padding: "16px 24px" }}>
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <Link href={`/admin/bundles/${p.id}`} style={{ width: 32, height: 32, borderRadius: 8, background: "white", border: "1px solid var(--gray-200)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--gray-900)", transition: "all 0.2s" }} title="Edit">
                      <Edit size={15} />
                    </Link>
